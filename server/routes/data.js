...
router.put('/addIdeaLike/:id', function(req, res){
  ...
  pool.connect(function (err, client, done) {
    client.query('SELECT * FROM ideas_likes WHERE user_id=4 AND idea_id=$1;', [ideaId], function(err, result){
      done();
      if(err){
...
router.put('/addIdeaLove/:id', function(req, res){
  var ideaId = req.params.id;
    // var userId = req.decodedToken.userSQLId;
  pool.connect(function (err, client, done) {
    client.query('SELECT * FROM ideas_loves WHERE user_id=4 AND idea_id=$1;', [ideaId], function(err, result){
      done();
      if(err){
